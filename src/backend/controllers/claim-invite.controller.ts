...
import { verifyRequest, isSuperAdminEmail, AuthError } from "@/backend/services/api-auth";
import { FieldValue } from "firebase-admin/firestore";

/**
 * POST /api/claim-invite
 * Se llama en cada login (desde el contexto de auth del frontend) con el ID
 * token del usuario. Usa el Admin SDK (bypassa las reglas de Firestore) para:
 ...
 *   3. Si el email (o su forma de cédula sintética) coincide con un armador
 *      ya registrado en el roster (colección `armadores`) → crea el perfil
 *      de armador directamente.
 ...
 * el cliente NO necesite leer Firestore directamente.
 */
export async function claimInvite(request: NextRequest) {
  try {
