                    ...
                    {isExpanded && (
                      <tr>
                        <td colSpan={8} style={{ padding: 0 }}>
                          <MembreteDetail membrete={m} zona={zona || undefined} armador={arm} zones={zones} user={user} />
                        </td>
                      </tr>
                    )}
                  </Fragment>
...
function MembreteDetail({ membrete, zona, armador, zones, user }: { membrete: Membrete; zona: Zone | undefined; armador: Armador | null; zones: Zone[]; user: { uid: string; name: string; companyId?: string } | null }) {
  const products = membrete.products || [];
  const totalCant = products.reduce((sum, p) => sum + p.cantidad, 0);
  const [editing, setEditing] = useState(false);
  const [editZonaCode, setEditZonaCode] = useState(membrete.zonaCode || "");
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);

  async function handleSaveZone() {
    if (!membrete.id || !user) return;
    const zona = zones.find((z) => z.code === editZonaCode);
    setSaving(true);
    try {
      await updateMembrete(membrete.id, {
        zonaId: zona?.id || "",
        zonaCode: editZonaCode,
      }, { uid: user.uid, name: user.name });
      setEditing(false);
      setSaved(true);
      setTimeout(() => setSaved(false), 2000);
    } catch (e) {
      console.error("Error updating membrete zone:", e);
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div style={{ padding: "16px 20px", background: "var(--panel2)", borderBottom: "1px solid var(--line)" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12, marginBottom: 16 }}>
        ...
        {/* Zona — editable */}
        <div>
          <div style={{ fontSize: 10, fontWeight: 600, color: "var(--faint)", textTransform: "uppercase", letterSpacing: ".04em", marginBottom: 2, display: "flex", alignItems: "center", gap: 6 }}>
            Zona
            {!editing && membrete.status !== "completed" && (
              <button
                onClick={() => { setEditing(true); setEditZonaCode(membrete.zonaCode || ""); }}
                style={{ fontSize: 9, padding: "1px 6px", borderRadius: 4, border: "1px solid var(--accent)", background: "transparent", color: "var(--accent)", cursor: "pointer", fontWeight: 600, textTransform: "uppercase" }}
              >
                Editar
              </button>
            )}
            {saved && <span style={{ fontSize: 9, color: "var(--s-done)", fontWeight: 600 }}>Guardado</span>}
          </div>
          {editing ? (
            <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
              <select
                value={editZonaCode}
                onChange={(e) => setEditZonaCode(e.target.value)}
                style={{ flex: 1, padding: "6px 8px", borderRadius: 6, border: "1px solid var(--accent)", background: "var(--bg)", fontSize: 13, fontWeight: 600 }}
              >
                <option value="">Sin zona</option>
                {zones.map((z) => <option key={z.id} value={z.code}>{z.code} — {z.sector}</option>)}
              </select>
              <button
                ...
                disabled={saving || editZonaCode === membrete.zonaCode}
                style={{ fontSize: 10, padding: "4px 8px", borderRadius: 4, border: "none", background: saving || editZonaCode === membrete.zonaCode ? "var(--faint)" : "var(--accent)", color: "#fff", cursor: saving || editZonaCode === membrete.zonaCode ? "default" : "pointer", fontWeight: 600 }}
              >
                {saving ? "..." : "OK"}
              </button>
              <button
                onClick={() => setEditing(false)}
                style={{ fontSize: 10, padding: "4px 8px", borderRadius: 4, border: "1px solid var(--line)", background: "var(--panel2)", cursor: "pointer" }}
              >
                X
              </button>
            </div>
          ) : (
            <div style={{ fontSize: 14, fontWeight: 600, color: membrete.zonaCode ? "var(--ink)" : "var(--faint)" }}>
              {membrete.zonaCode || "\u2014"}
            </div>
          )}
        </div>
