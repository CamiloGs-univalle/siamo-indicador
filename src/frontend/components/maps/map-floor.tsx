 ...
 * del plano físico real de la bodega (`@/frontend/services/warehouse-floorplan`) — zona de
...
import { WAREHOUSE_FLOORPLAN_BLOCKS, WAREHOUSE_FLOORPLAN_LEGEND, floorplanBlockColor } from "@/frontend/services/warehouse-floorplan";
...
interface MapFloorProps {
  ...
  /**
   * Mapa de calor: intensidad de carga de trabajo de la zona, como un
   * bucket 0-4 (0 = sin carga, 4 = crítica — ver heatBucket() en
   * mod-mapa.tsx). Solo se usa cuando heatEnabled es true; si no se
   * provee, la ficha se ve exactamente igual que antes (sin teñir).
   */
  heatOf?: (code: string) => number;
  /** Activa/desactiva el teñido de calor sin tener que quitar heatOf. */
  heatEnabled?: boolean;
  /**
...
export function MapFloor({
  ...
  // Núcleo del encuadre automático, extraído a función para poder llamarlo
  // tanto cuando cambia el conjunto de zonas como cuando el CONTENEDOR
  // cambia de tamaño (p. ej. al arrastrar el divisor entre "Mapa de la
  // bodega" y "Monitor de zonas") — usa siempre refs (nunca las props
  // capturadas en el closure) para no quedar con datos viejos sin
  // importar cuándo se creó la función.
  const doFit = () => {
    const el = ref.current;
    const currentCodes = codesRef.current;
    if (!el || currentCodes.length === 0) return;
  ...
  // Reencuadra automáticamente si el contenedor cambia de tamaño (p. ej. al
  // arrastrar el divisor entre "Mapa de la bodega" y "Monitor de zonas", o
  // al redimensionar la ventana) — así el plano SIEMPRE se ve completo y
  // proporcionado al espacio real disponible, en vez de quedar cortado con
  // zonas fuera de vista y sin forma de llegar a ellas.
  useEffect(() => {
    const el = ref.current;
    if (!el) return;
      ...
      <div className={"floor" + (editable ? " edit" : "")} ref={ref}>
        <div className="floor-zoom" style={{ transform: `scale(${zoom})`, transformOrigin: "0 0" }}>
          ...
          {codes.map((code) => {
            ...
            // Bucket de calor (0 = sin teñir): independiente del status/
            // armador — se agrega como clase adicional, así que nunca pisa
            // la franja .strip ni la bolita .sdot.
            const heatBucketCls = heatEnabled && heatOf ? heatOf(code) : 0;
            return (
              <div
                key={code}
                className={
                  ...
                  (fixed ? " fixed" : "") +
                  (compact ? " compact" : "") +
                  (heatBucketCls > 0 ? " heat-" + heatBucketCls : "") +
                  " " + zoneStatus
                }
